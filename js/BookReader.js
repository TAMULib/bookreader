...
(function ($) {
    ...
    $(window).bind("resize", this, function (e) {
      ...
      if (1 == e.data.mode) {
        ...
        if (e.data.twoPage.autofit) {
          ...
          if (
            e.data.twoPage.totalWidth < $("#BRcontainer").attr("clientWidth")
          ) {
            center.percentageX = 0.5;
            doRecenter = true;
          }
          if (
            e.data.twoPage.totalHeight < $("#BRcontainer").attr("clientHeight")
          ) {
  ...
  BookReader.prototype.drawLeafsOnePage = function () {
    ...
    for (i = 0; i < indicesToDisplay.length; i++) {
      ...
      if (
        BookReader.util.notInArray(indicesToDisplay[i], this.displayedIndices)
      ) {
    ...
    for (i = 0; i < this.displayedIndices.length; i++) {
      if (
        BookReader.util.notInArray(this.displayedIndices[i], indicesToDisplay)
      ) {
  ...
  BookReader.prototype.drawLeafsThumbnail = function (seekIndex) {
    ...
    for (i = 0; i < this.numLeafs; i++) {
      ...
      leafHeight = parseInt(
        (this.getPageHeight(leafMap[currentRow].leafs[leafIndex].num) *
          this.thumbWidth) /
    ...
    var pageViewBuffer =
      Math.floor(($("#BRcontainer").attr("scrollWidth") - maxRight) / 2) - 14;
    ...
    for (i = 0; i < rowsToDisplay.length; i++) {
      if (BookReader.util.notInArray(rowsToDisplay[i], this.displayedRows)) {
        ...
        for (j = 0; j < leafMap[row].leafs.length; j++) {
          ...
          leafWidth = this.thumbWidth;
          leafHeight = parseInt(
            (this.getPageHeight(leaf) * this.thumbWidth) /
              this.getPageWidth(leaf),
            10
          );
          ...
          var thumbReduce = Math.floor(
            this.getPageWidth(leaf) / this.thumbWidth
          );
  ...
  BookReader.prototype.zoom1up = function (direction) {
    ...
    var reduceFactor = this.nextReduce(
      this.reduce,
      direction,
  ...
  BookReader.prototype.resizePageView1up = function () {
    ...
    if (oldScrollTop > 0) {
      ...
      var fudgeFactor =
        (this.getPageHeight(this.currentIndex()) / this.reduce) * 0.6;
      var oldLeafTop =
        this.onePageGetPageTop(this.currentIndex()) + fudgeFactor;
      var oldViewDimensions = this.onePageCalculateViewDimensions(
        this.reduce,
        this.padding
      );
    ...
    var viewDimensions = this.onePageCalculateViewDimensions(
      this.reduce,
      this.padding
    );
    ...
    var newTop = Math.max(
      0,
      Math.floor(newCenterY - $("#BRcontainer").height() / 2)
    );
  ...
  BookReader.prototype.onePageCalculateViewDimensions = function (
    reduce,
    padding
  ) {
    var viewWidth = 0;
    ...
    for (i = 0; i < this.numLeafs; i++) {
      viewHeight +=
        parseInt(this._getPageHeight(i) / this.reduce) + this.padding;
  ...
  BookReader.prototype.centerX1up = function () {
    var centerX;
    if ($("#BRpageview").width() < $("#BRcontainer").attr("clientWidth")) {
      // fully shown
      centerX = $("#BRpageview").width();
    } else {
      centerX =
        $("#BRcontainer").attr("scrollLeft") +
        $("#BRcontainer").attr("clientWidth") / 2;
    }
  ...
  BookReader.prototype.centerY1up = function () {
    var centerY =
      $("#BRcontainer").attr("scrollTop") + $("#BRcontainer").height() / 2;
  ...
  BookReader.prototype.nextReduce = function (
    currentReduce,
    direction,
    reductionFactors
  ) {
  ...
  BookReader.prototype.jumpToIndex = function (index, pageX, pageY) {
    ...
    if (this.constMode2up == this.mode) {
      ...
      // is current
      if (
        index < Math.min(this.twoPage.currentIndexL, this.twoPage.currentIndexR)
      ) {
        this.flipBackToIndex(index);
      } else if (
        index > Math.max(this.twoPage.currentIndexL, this.twoPage.currentIndexR)
      ) {
      ...
      for (i = 0; i < index + 1; i++) {
        ...
        leafHeight = parseInt(
          (this.getPageHeight(leafIndex) * this.thumbWidth) /
            this.getPageWidth(leafIndex),
          10
        );
      ...
      //$('#BRcontainer').attr('scrollTop', leafTop);
      $("#BRcontainer").animate(
        { scrollTop: leafTop, scrollLeft: leafLeft },
        "fast"
      );
    }
  };
  ...
  BookReader.prototype.switchMode = function (mode) {
    ...
    if (1 == mode) {
      ...
      this.reduce = this.quantizeReduce(
        this.reduce,
        this.onePage.reductionFactors
      );
      this.prepareOnePageView();
  ...
  BookReader.prototype.prepareTwoPageView = function (
    centerPercentageX,
    centerPercentageY
  ) {
    $("#BRcontainer").empty();
  ...
  BookReader.prototype.prepareTwoPagePopUp = function () {
    ...
    $(this.leafEdgeR).bind("mousemove", this, function (e) {
      ...
      $(e.data.twoPagePopUp).css({
        left:
          e.pageX -
          $("#BRcontainer").offset().left +
          $("#BRcontainer").scrollLeft() -
          100 +
          "px",
        top:
          e.pageY -
          $("#BRcontainer").offset().top +
          $("#BRcontainer").scrollTop() +
          "px",
      });
    });
  ...
  BookReader.prototype.calculateSpreadSize = function () {
    ...
    if (this.twoPage.autofit) {
      ...
      spreadSize = this.getSpreadSizeFromReduce(
        firstIndex,
        secondIndex,
        this.reduce
      );
    ...
    this.twoPage.leafEdgeWidthL = this.leafEdgeWidth(
      this.twoPage.currentIndexL
    );
    this.twoPage.leafEdgeWidthR =
      this.twoPage.edgeWidth - this.twoPage.leafEdgeWidthL;
    ...
    this.twoPage.bookCoverDivHeight =
      this.twoPage.height + 2 * this.twoPage.coverInternalPadding;

    // We calculate the total width and height for the div so that we can make the book
    // spine centered
    var leftGutterOffset = this.gutterOffsetForIndex(firstIndex);
    var leftWidthFromCenter =
      this.twoPage.scaledWL - leftGutterOffset + this.twoPage.leafEdgeWidthL;
    var rightWidthFromCenter =
      this.twoPage.scaledWR + leftGutterOffset + this.twoPage.leafEdgeWidthR;
    var largestWidthFromCenter = Math.max(
      leftWidthFromCenter,
    ...
    this.twoPage.totalHeight =
      this.twoPage.height +
      2 *
        (this.twoPage.coverInternalPadding + this.twoPage.coverExternalPadding);
    ...
    this.twoPage.gutter =
      this.twoPage.middle + this.gutterOffsetForIndex(firstIndex);
    ...
    // Book spine
    this.twoPage.bookSpineDivHeight =
      this.twoPage.height + 2 * this.twoPage.coverInternalPadding;
    this.twoPage.bookSpineDivLeft =
      this.twoPage.middle - (this.twoPage.bookSpineDivWidth >> 1);
  ...
  BookReader.prototype.getIdealSpreadSize = function (firstIndex, secondIndex) {
    ...
    var ratio;
    if (
      Math.abs(firstIndexRatio - canon5Dratio) <
      Math.abs(secondIndexRatio - canon5Dratio)
    ) {
    ...
    var maxLeafEdgeWidth = parseInt(
      $("#BRcontainer").attr("clientWidth") * 0.1
    );
  ...
  BookReader.prototype.getSpreadSizeFromReduce = function (
    firstIndex,
    secondIndex,
    reduce
  ) {
    ...
    spreadSize.totalLeafEdgeWidth = Math.min(
      totalLeafEdgeWidth,
      maxLeafEdgeWidth
    );
    ...
    var nativeWidth =
      this._getPageWidth(firstIndex) + this._getPageWidth(secondIndex);
    var nativeHeight =
      this._getPageHeight(firstIndex) + this._getPageHeight(secondIndex);
  ...
  BookReader.prototype.twoPageGetAutofitReduce = function () {
    var spreadSize = this.getIdealSpreadSize(
      this.twoPage.currentIndexL,
      this.twoPage.currentIndexR
    );
  ...
  BookReader.prototype.onePageGetAutofitWidth = function () {
    ...
    return (
      ...
      ($("#BRcontainer").attr("clientWidth") - widthPadding * 2)
    );
  };

  BookReader.prototype.onePageGetAutofitHeight = function () {
    return (
      (this.getMedianPageSize().height + 0.0) /
      ($("#BRcontainer").attr("clientHeight") - this.padding * 2)
  ...
  BookReader.prototype.twoPageCalculateReductionFactors = function () {
    this.twoPage.reductionFactors = this.reductionFactors.concat([
      {
        reduce: this.getIdealSpreadSize(
          this.twoPage.currentIndexL,
          this.twoPage.currentIndexR
        ).reduce,
        autofit: "auto",
  ...
  BookReader.prototype.twoPageSetCursor = function () {
    ...
    if (
      $("#BRtwopageview").width() > $("#BRcontainer").attr("clientWidth") ||
  ...
  BookReader.prototype.scrollDown = function () {
    if ($.inArray(this.mode, [this.constMode1up, this.constModeThumb]) >= 0) {
      if (
        this.mode == this.constMode1up &&
        this.reduce >= this.onePageGetAutofitHeight()
      ) {
  ...
  BookReader.prototype.flipBackToIndex = function (index) {
    ...
    if ("rl" != this.pageProgression) {
      ...
      var gutter = this.prepareFlipRightToLeft(
        previousIndices[0],
        previousIndices[1]
      );
  ...
  BookReader.prototype.flipLeftToRight = function (newIndexL, newIndexR) {
    ...
    var right =
      $("#BRtwopageview").attr("clientWidth") -
      left -
      $(this.prefetchedImgs[leftLeaf]).width() +
      $("#BRtwopageview").offset().left -
      2 +
      "px";
          ...
          function () {
            ...
            $(self.twoPage.coverDiv).css({
              ...
              left:
                ...
                newWidthL -
                newLeafEdgeWidthL -
                self.twoPage.coverInternalPadding +
                "px",
            });
  ...
  BookReader.prototype.pruneUnusedImgs = function () {
    ...
    for (var key in this.prefetchedImgs) {
      ...
      if (
        key != this.twoPage.currentIndexL &&
        key != this.twoPage.currentIndexR
      ) {
        //console.log('removing key '+ key);
        $(this.prefetchedImgs[key]).remove();
      }
      if (
        key < this.twoPage.currentIndexL - 4 ||
        key > this.twoPage.currentIndexR + 4
      ) {
  ...
  BookReader.prototype.prefetch = function () {
    ...
    var lowCurrent = Math.min(
      this.twoPage.currentIndexL,
      this.twoPage.currentIndexR
    );
    var highCurrent = Math.max(
      this.twoPage.currentIndexL,
      this.twoPage.currentIndexR
    );
  ...
  BookReader.prototype.search = function (term) {
    ...
    url +=
      "&path=" +
      this.bookPath.replace(new RegExp("/" + this.subPrefix + "$"), ""); //remove subPrefix from end of path
    ...
    this.showProgressPopup(
      '<img id="searchmarker" src="' +
        this.imagesBaseURL +
        "marker_srch-on.png" +
        '"> Search results will appear below... <br/><br/>(Search results may be reduced due to the accuracy of OCR)<br/><br/>',
      xhr
    );
  };
  ...
  BookReader.prototype.BRSearchCallback = function (results) {
    ...
    if (0 == results.matches.length) {
      var errStr =
        "No matches were found.<br/><br/>(Search results may be reduced due to the accuracy of OCR)";
      var timeout = 4000;
      if (false === results.indexed) {
        errStr =
          "<p>This book hasn't been indexed for searching yet. We've just started indexing it, so search should be available soon. Please try again later. Thanks!</p>";
        timeout = 5000;
      }
    ...
    for (i = 0; i < results.matches.length; i++) {
      if (results.matches[i].par[0] != null) {
        br.addSearchResult(
          ...
          br.leafNumToIndex(results.matches[i].par[0].page)
        );
      }
    }
      ...
      for (j = 0; j < results.matches[i].par[0].boxes.length; j++) {
        ...
        if (jQuery.inArray(pageIndex, this.displayedIndices) >= 0) {
          if (null == box.div) {
            ...
            $(box.div)
              .attr("className", "BookReaderSearchHilite")
              .appendTo("#pagediv" + pageIndex);
          }
          $(box.div).css({
  ...
  BookReader.prototype.twoPageGutter = function () {
    return (
      this.twoPage.middle +
      this.gutterOffsetForIndex(this.twoPage.currentIndexL)
    );
  };
  ...
  BookReader.prototype.twoPageTop = function () {
    return (
      this.twoPage.coverExternalPadding + this.twoPage.coverInternalPadding
    ); // $$$ + border?
  };
  ...
  BookReader.prototype.twoPageCoverWidth = function (totalPageWidth) {
    return (
      totalPageWidth +
      this.twoPage.edgeWidth +
      2 * this.twoPage.coverInternalPadding
    );
  };
  ...
  BookReader.prototype.twoPageGetViewCenter = function () {
    ...
    center.percentageX =
      (containerOffset.left -
        viewOffset.left +
        ($("#BRcontainer").attr("clientWidth") >> 1)) /
      this.twoPage.totalWidth;
    center.percentageY =
      (containerOffset.top -
        viewOffset.top +
        ($("#BRcontainer").attr("clientHeight") >> 1)) /
      this.twoPage.totalHeight;
  ...
  BookReader.prototype.twoPageCenterView = function (percentageX, percentageY) {
    ...
    if (viewWidth < containerClientWidth) {
      ...
      $("#BRtwopageview").css(
        "left",
        (containerClientWidth >> 1) - intoViewX + "px"
      );
    } else {
    ...
    if (viewHeight < containerClientHeight) {
      ...
      $("#BRtwopageview").css(
        "top",
        (containerClientHeight >> 1) - intoViewY + "px"
      );
    } else {
  ...
  BookReader.prototype.twoPageFlipAreaTop = function () {
    return parseInt(
      this.twoPage.bookCoverDivTop + this.twoPage.coverInternalPadding
    );
  };
  ...
  BookReader.prototype.twoPageRightFlipAreaLeft = function () {
    return parseInt(
      this.twoPage.gutter + this.twoPage.scaledWR - this.twoPageFlipAreaWidth()
    );
  };
  ...
  BookReader.prototype.setHilightCss2UP = function (
    div,
    index,
    left,
    right,
  ...
  BookReader.prototype.getPrintURI = function () {
    ...
    var options =
      ...
      this.imageFormat +
      "&file=" +
      this._getPageFile(indexToPrint) +
      "&width=" +
      this._getPageWidth(indexToPrint) +
      "&height=" +
      this._getPageHeight(indexToPrint);

    if (this.constMode2up == this.mode) {
      options +=
        "&file2=" +
        this._getPageFile(this.twoPage.currentIndexR) +
        "&width2=" +
        this._getPageWidth(this.twoPage.currentIndexR);
      ...
      options +=
        ...
        encodeURIComponent(
          this.shortTitle(50) +
            " - Pages " +
            this.getPageNum(this.twoPage.currentIndexL) +
            ", " +
            this.getPageNum(this.twoPage.currentIndexR)
        );
    } else {
      options +=
        "&title=" +
        encodeURIComponent(
          this.shortTitle(50) + " - Page " + this.getPageNum(indexToPrint)
        );
    }
  ...
  BookReader.prototype.showEmbedCode = function () {
    ...
    htmlStr =
      '<h3 style="background:#615132;padding:10px;margin:0 0 10px;color:#fff;">Embed Bookreader</h3>';
    htmlStr +=
      '<textarea rows="2" cols="40" style="margin-left:10px;width:368px;height:40px;color:#333;font-size:12px;border:2px inset #ccc;background:#efefef;padding:2px;-webkit-border-radius:4px;-moz-border-radius:4px;border-radius:4px;">' +
      this.getEmbedCode() +
      "</textarea>";
    htmlStr +=
      '<a href="javascript:;" class="popOff" onclick="$(this.parentNode).remove();$(\'.coverUp\').hide();return false" style="color:#999;"><span>Close</span></a>';

    this.embedPopup.innerHTML = htmlStr;
    $("#BookReader").append(
      '<div class="coverUp" style="position:absolute;z-index:299;width:100%;height:100%;background:#000;opacity:.4;filter:alpha(opacity=40);" onclick="$(\'.popped\').hide();$(this).hide();"></div>'
    );
    $(this.embedPopup)
      .find("textarea")
      .click(function () {
  ...
  BookReader.prototype.showBookmarkCode = function () {
    ...
    htmlStr =
      '<h3 style="background:#615132;padding:10px;margin:0 0 10px;color:#fff;">Add a bookmark</h3>';
    htmlStr +=
      '<p style="padding:10px;line-height:18px;">You can add a bookmark to any page in any book. If you elect to make your bookmark public, other readers will be able to see it. <em>You must be logged in to your <a href="">Open Library account</a> to add bookmarks.</em></p>';
    htmlStr +=
  ...
  BookReader.prototype.autoToggle = function () {
    ...
    if (null == this.autoTimer) {
      ...
      this.autoTimer = setInterval(function () {
        ...
        if (
          Math.max(self.twoPage.currentIndexL, self.twoPage.currentIndexR) >=
          self.lastDisplayableIndex()
        ) {
  ...
  BookReader.prototype.gutterOffsetForIndex = function (pindex) {
    ...
    var offset = parseInt(
      (pindex / this.numLeafs - 0.5) * this.twoPage.edgeWidth
    );
  ...
  BookReader.prototype.leafEdgeWidth = function (pindex) {
    ...
    if (this.getPageSide(pindex) == "L" && this.pageProgression != "rl") {
      ...
      return parseInt(
        (1 - pindex / this.numLeafs) * this.twoPage.edgeWidth + 0.5
      );
    }
  };
  ...
  BookReader.prototype.jumpIndexForLeftEdgePageX = function (pageX) {
    if ("rl" != this.pageProgression) {
      ...
      var jumpIndex =
        this.twoPage.currentIndexL -
        ($(this.leafEdgeL).offset().left + $(this.leafEdgeL).width() - pageX) *
          10;
  ...
  BookReader.prototype.jumpIndexForRightEdgePageX = function (pageX) {
    if ("rl" != this.pageProgression) {
      ...
      var jumpIndex =
        this.twoPage.currentIndexR +
        (pageX - $(this.leafEdgeR).offset().left) * 10;
      jumpIndex = BookReader.util.clamp(
        Math.round(jumpIndex),
        this.twoPage.currentIndexR + 2,
        this.lastDisplayableIndex()
      );
      return jumpIndex;
    } else {
  ...
  BookReader.prototype.initNavbar = function () {
    ...
    var handleHelper = $("#BRpager .ui-slider-handle").append(
      '<div id="pagenum"><span class="currentpage"></span></div>'
    );
  ...
  BookReader.prototype.addSearchResult = function (queryString, pageIndex) {
    ...
    var percentThrough = BookReader.util.cssPercentage(
      pageIndex,
      this.numLeafs - 1
    );
    ...
    queryString = queryString.replace(
      re,
      '<a href="#" onclick="br.jumpToIndex(' +
        pageIndex +
        '); return false;">$1</a>'
    );

    var marker = $(
      '<div class="search" style="top:' +
        -$("#BRcontainer").height() +
        "px; left:" +
        percentThrough +
        ';" title="' +
  ...
  BookReader.prototype.addChapter = function (
    chapterTitle,
    pageNumber,
    pageIndex
  ) {
  ...
  BookReader.prototype.addChapterFromEntry = function (tocEntryObject) {
    ...
    if (pageIndex) {
      this.addChapter(
        tocEntryObject["title"],
        tocEntryObject["pagenum"],
        pageIndex
      );
    }
  ...
  BookReader.prototype.initToolbar = function (mode, ui) {
      ...
      if (this.bookId.indexOf("yellbooks") == 0) {
        ...
        $("#BookReader").append(
          "<div id='BRtoolbar'>" +
            ...
            "<span><a class='logotamu' href='https://www.tamu.edu/'></a><a class='logo' href='" +
            this.logoURL +
            ...
            this.bookTitle +
            " " +
            this.bookId.replace("yb", "").replace("yellbooks_", "") +
            "</li></ul></span>" +
    ...
    if (
      ...
      $.browser.webkit &&
      parseInt($.browser.version, 10) <= 531
    ) {
    ...
    if (
      !(
        this.canSwitchToMode(this.constMode2up) ||
        this.canSwitchToMode(this.constModeThumb)
      )
    ) {
    ...
    jToolbar.find(".info").colorbox({
      ...
      onClosed: function () {
        jToolbar.find(".info").focus();
      },
    });
    jToolbar.find(".question").colorbox({
      inline: true,
      opacity: "0.5",
      href: "#BRquestion",
      onLoad: function () {
      ...
      onComplete: function () {
        $("#BRquestionTitle").focus();
      },
      onClosed: function () {
        jToolbar.find(".question").focus();
      },
    });
    ...
    $('<div style="display: none;"></div>')
      ...
      .append(this.blankQuestionDiv())
      .appendTo($("body"));
    // $('<div style="display: none;"></div>').append(this.blankInfoDiv()).appendTo($('body'));
  ...
  BookReader.prototype.bindNavigationHandlers = function () {
    ...
    $(".BRnavCntl").click(function () {
      if ($("#BRnavCntlBtm").hasClass("BRdn")) {
        ...
        $(".BRnavCntl").delay(1000).animate({ opacity: 0.25 }, 1000);
      } else {
    ...
    $("#BRnavCntlBtm").mouseleave(function () {
      if ($(this).hasClass("BRup")) {
        $(".BRnavCntl").animate({ opacity: 0.25 }, 250);
      }
    });
    ...
    $(".BRpageimage")
      .die("mousedown.swipe")
      .live("mousedown.swipe", { br: this }, this.swipeMousedownHandler);
  ...
  BookReader.prototype.swipeMousemoveHandler = function (event) {
    ...
    if (_swipe.deltaT > maxSwipeTime && !_swipe.didSwipe) {
      if (_swipe.mightBeDragging) {
        ...
        $("#BRcontainer")
          .scrollTop(
            $("#BRcontainer").scrollTop() - event.clientY + _swipe.lastY
          )
          .scrollLeft(
            $("#BRcontainer").scrollLeft() - event.clientX + _swipe.lastX
          );
      }
    }
  ...
  BookReader.prototype.paramsFromFragment = function (urlFragment) {
    ...
    if (urlHash["search"] != undefined) {
      params.searchTerm = BookReader.util.decodeURIComponentPlus(
        urlHash["search"]
      );
    }
  ...
  BookReader.prototype.fragmentFromParams = function (params) {
    ...
    return BookReader.util
      .encodeURIComponentPlus(fragments.join(separator))
      .replace(/%2F/g, "/");
  };
  ...
  BookReader.prototype.gotOpenLibraryRecord = function (self, olObject) {
    ...
    if (olObject) {
      ...
      if (self.olAuth) {
        ...
        $(
          '<form id="BRreturnform" action="' +
            returnUrl +
            '" method="post"><input type="submit" value="Return book" onclick="olAuth.deleteCookies();"/><input type="hidden" name="action" value="return" /></form>'
        ).appendTo("#BRreturn");
      } else {
        $("<a/>")
          .attr({
      ...
      if (soundManager.supported()) {
        ...
        soundManager.onready(function (oStatus) {
          if (oStatus.success) {
            ...
            alert(
              "Could not load soundManager2, possibly due to FlashBlock. Audio playback is disabled"
            );
          }
        }, this);
      }
  ...
  BookReader.prototype.ttsGetText = function (index, callback) {
    var url =
      "https://" +
      this.server +
      "/BookReader/BookReaderGetTextWrapper.php?path=" +
      this.bookPath +
      "_djvu.xml&page=" +
      index;
    this.ttsAjax = $.ajax({
  ...
  BookReader.prototype.ttsStartCB = function (data) {
    ...
    var snd = soundManager.createSound({
      ...
      url:
        "https://" +
        this.server +
        "/BookReader/BookReaderGetTTS.php?string=" +
        escape(data[0][0]) +
        "&format=." +
        this.ttsFormat, //the .ogg is to trick SoundManager2 to use the HTML5 audio player
      onload: function () {
  ...
  BookReader.prototype.showOpenSeaDragon = function (page) {
    ...
    var url = isNaN(page)
      ? page
      : "https://bookreader.library.tamu.edu/BookReader/BookReaderImages.php?zip=" +
        this.zip +
        "&file=" +
        this._getPageFile(page) +
        "&scale=4&rotate=0";
  ...
  BookReader.prototype.ttsNextChunk = function () {
    ...
    if (-1 != this.ttsPosition) {
      soundManager.destroySound(
        "chunk" + this.ttsIndex + "-" + this.ttsPosition
      );
    }
    ...
    if (this.ttsPosition >= this.ttsChunks.length) {
      if (this.ttsIndex == this.numLeafs - 1) {
        ...
        if (null != this.ttsNextChunks || starting) {
          ...
          if (2 == this.mode) {
            if (
              this.ttsIndex != this.twoPage.currentIndexL &&
              this.ttsIndex != this.twoPage.currentIndexR
            ) {
              if (!starting) {
  ...
  BookReader.prototype.ttsPrefetchAudio = function () {
    ...
    if (nextPos < this.ttsChunks.length) {
      ...
      if (soundManager.debugMode)
        console.log(
          "preloading chunk 0 from next page, index=" + (this.ttsIndex + 1)
        );
  ...
  BookReader.prototype.ttsScrollToChunk = function (chunk) {
    ...
    if (soundManager.debugMode)
      console.log(
        "containerTop = " + containerTop + " containerBot = " + containerBot
      );
  ...
  BookReader.prototype.ttsHilite1UP = function (chunk) {
    ...
    for (i = 1; i < chunk.length; i++) {
      ...
      $(div)
        .attr("className", "BookReaderSearchHilite")
        .appendTo("#pagediv" + this.ttsIndex);
  ...
  BookReader.prototype.buildShareDiv = function (jShareDiv) {
    ...
    var jForm = $(
      [
        ...
        '<input type="text" name="pageview" id="pageview" value="' +
          pageView +
          '"/>',
        "</fieldset>",
        "<fieldset>",
        ...
        '<input type="text" name="booklink" id="booklink" value="' +
          bookView +
          '"/>',
        "</fieldset>",
        "<fieldset>",
        ...
        '<label class="sub">',
        '<input type="radio" name="pages" value="' +
          this.constMode1up +
          '" checked="checked"/>',
        "1 page",
        "</label>",
    ...
    jForm.find("input").bind("change", function () {
      ...
      form
        .find(".BRframeEmbed")
        .val(self.getEmbedCode(frameWidth, frameHeight, params));
    });
  ...
  BookReader.prototype.buildInfoDiv = function (jInfoDiv) {
    jInfoDiv
      .find(".BRfloatTitle a")
      .attr({ href: this.bookUrl, alt: this.bookTitle })
      .text(this.bookTitle);
  };
  ...
  BookReader.prototype.initUIStrings = function () {
    ...
    var titles = {
      ".logo": "Go to library.tamu.edu", // $$$ update after getting OL record
      ".zoom_in": "Zoom in",
      ".zoom_out": "Zoom out",
      ".onepg": "One-page view",
      ".twopg": "Two-page view",
