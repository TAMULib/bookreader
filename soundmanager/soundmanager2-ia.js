...
/* This version of SoundManager contains a modification on lines 214-219 for 
   use in the Internet Archive Bookreader (http://openlibrary.org/dev/docs/bookreader).
   For Firefox browsers that support the audio element, we don't load the
   flash widget, since we only play ogg audio for Firefox users.

   Also, line 2618 has been modified to disable HTML5 audio on all combinations
   of Safari and OS X 10.6. Previously, HTML5 audio was only being disabled on
   Safari on 10.6.3 and 10.6.4.
   
   Modifications are marked with "IA modification:".
*/
...
function SoundManager(smURL, smID) {
  ...
  // IA modification: do not load the flash widget on firefox, which supports
  // native ogg playback. We don't need to play any other file types, and
  // some users are having trouble disabling flashblock.
  var _is_firefox = _ua.match(/firefox/i) && (typeof Audio !== 'undefined');

  if (_iPadOrPhone || _is_pre || _is_firefox) {
  ...
  _featureCheck = function() {
    var needsFlash, item,
    isBadSafari = (!_wl.match(/usehtml5audio/i) && !_wl.match(/sm2\-ignorebadua/i) && _isSafari && _ua.match(/OS X 10_6/i)), //IA modification: disable on all OS X 10.6, was: _ua.match(/OS X 10_6_(3|4)/i)), // Safari 4 and 5 occasionally fail to load/play HTML5 audio on Snow Leopard due to bug(s) in QuickTime X and/or other underlying frameworks. :/ Known Apple "radar" bug. https://bugs.webkit.org/show_bug.cgi?id=32159
